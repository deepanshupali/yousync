...
export default function Player({
  ...
  // 🔹 NEW: control feedback loops
  const suppressNextOnStateChange = useRef(false);
  ...
  const onReady: YouTubeProps["onReady"] = async (event) => {
    ...
    try {
      ...
      if (position > 0) {
        console.log(`⏩ Seeking to ${position}s on ready`);
        suppressNextOnStateChange.current = true; // prevent loop from this seek
        event.target.seekTo(position, true);
      }

      if (playing) {
        suppressNextOnStateChange.current = true;
        event.target.playVideo();
      } else {
        suppressNextOnStateChange.current = true;
        event.target.pauseVideo();
  ...
  // 🔁 Sync effect when props (playing/position) change from server
  useEffect(() => {
    if (!isReady || !playerRef.current) return;

    // If this change came from the admin's own PATCH, ignore it once
    ...
    try {
      ...
      // Only seek when actually desynced
      if (diff > 1) {
        suppressNextOnStateChange.current = true; // this seek will trigger onStateChange
        player.seekTo(position, true);
      }
  ...
  const updateState = async (newPlaying: boolean, pos?: number) => {
    ...
    const currentTime = pos ?? playerRef.current?.getCurrentTime();

    console.log(
      "Updating state. Playing:",
      newPlaying,
      "Position:",
      currentTime
    );

    // 🔹 this PATCH originates from this client → ignore next sync echo
  ...
  const onStateChange: YouTubeProps["onStateChange"] = (event) => {
    ...
    // 🔹 Ignore the event that we *know* was caused by our own seek/play/pause
    if (suppressNextOnStateChange.current) {
      suppressNextOnStateChange.current = false;
      return;
    }
    ...
    if (isAdmin) {
      switch (event.data) {
        ...
        case YouTube.PlayerState.PAUSED:
          updateState(false, currentTime);
          break;
        // 🔸 BUFFERING updates removed to avoid thrashing while seeking
        case YouTube.PlayerState.ENDED:
          updateState(false, currentTime);
          break;
      }
    } else {
      // Members should NOT fight the server; only gently correct
      switch (event.data) {
        case YouTube.PlayerState.PLAYING:
          if (!playing) {
            suppressNextOnStateChange.current = true;
        ...
        case YouTube.PlayerState.PAUSED:
          ...
          break;
        case YouTube.PlayerState.BUFFERING:
          // Let the sync effect handle seeking; don't fight it here
          break;
      }
    }
  };
